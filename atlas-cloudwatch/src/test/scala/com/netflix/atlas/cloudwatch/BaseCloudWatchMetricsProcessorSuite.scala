...
class BaseCloudWatchMetricsProcessorSuite extends FunSuite with TestKitBase with ImplicitSender {
  ...
  val category = MetricCategory("ut", "AWS/DynamoDB", 60, -1, List("MyTag"), List.empty, null)
  val category5m = MetricCategory("ut", "AWS/DynamoDB", 300, -1, List("MyTag"), List.empty, null)
